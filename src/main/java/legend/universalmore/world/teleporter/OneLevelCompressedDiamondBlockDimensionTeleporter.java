...
public class OneLevelCompressedDiamondBlockDimensionTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("one_level_compressed_diamond_block_dimension_portal",
			Vec3i::compareTo, 300);
	...
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(ForgeRegistries.Keys.POI_TYPES, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(
					UniversalmoreModBlocks.ONE_LEVEL_COMPRESSED_DIAMOND_BLOCK_DIMENSION_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register("one_level_compressed_diamond_block_dimension_portal", poiType);
	...
	public OneLevelCompressedDiamondBlockDimensionTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, UniversalmoreModBlocks.ONE_LEVEL_COMPRESSED_DIAMOND_BLOCK.get().defaultBlockState(),
							3);
				}
			}
		}
		BlockState blockstate = UniversalmoreModBlocks.ONE_LEVEL_COMPRESSED_DIAMOND_BLOCK_DIMENSION_PORTAL.get().defaultBlockState()
				.setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = OneLevelCompressedDiamondBlockDimensionPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis,
						entity.position(), entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return OneLevelCompressedDiamondBlockDimensionPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
